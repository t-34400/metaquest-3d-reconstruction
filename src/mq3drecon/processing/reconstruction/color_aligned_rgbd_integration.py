...
def _load_rgbd_images(
    ...
    if color.shape != expected_shape:
        raise ValueError(
            "Color image shape does not match color-aligned depth resolution: "
...
def reconstruct_color_aligned_rgbd_scene(data_io: DataIO, config: ReconstructionConfig) -> None:
    ...
    if config.optimize_color_pose:
        print("[Info] Skipping color map optimization for color_aligned depth source; integrating LEFT RGBD directly.")
    ...
    if config.use_colorless_vbg_cache:
        print("[Info] Ignoring colorless VBG cache for color_aligned RGBD integration.")
    ...
    for index in tqdm(range(len(depth_dataset)), desc="[LEFT] Integrating color-aligned RGBD frames ..."):
